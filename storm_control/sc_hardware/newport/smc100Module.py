#!/usr/bin/env python
"""
HAL module for controlling a Newport stage (single axis actuator attached to a plate).
...
class NewportStageFunctionality(stageModule.StageFunctionality):
    """
    ...
    def __init__(self, update_interval = None, **kwds):
        ...
        self.polling_thread = NewportPollingThread(device_mutex = self.device_mutex,
...
class NewportStage(stageModule.StageModule):

    def __init__(self, module_params = None, qt_settings = None, **kwds):
        super().__init__(**kwds)

        configuration = module_params.get("configuration")
        self.stage = SMC100.SMC100(baudrate = configuration.get("baudrate"),
